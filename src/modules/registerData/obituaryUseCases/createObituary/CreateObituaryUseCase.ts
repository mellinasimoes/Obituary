...
class CreateObituaryUseCase{
  ...
  async execute ({person_id,
    ...
    const deathCertificateAlreadyExists = await this.obituaryRepository.findObituaryByDeathCertificate(death_certificate);
    ...
    const obituary =  await this.obituaryRepository.create({person_id,
       death_date,
       block,
       temporary_grave_number,
       final_grave_number,
